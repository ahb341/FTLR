...
void commsKeystrokeTestHarness(char keypress)
{
  ...
  switch (keypress)
  {
    case '/':
    {
      toPost.EventType = ES_PairingRequest;
      PostTUGCommComm(toPost);
      break;
    }

    case '.':
    {
      toPost.EventType = ES_ControlPacketRecd;
      PostTUGCommComm(toPost);
      break;
    }
    ...
    case ',':
    {
      toPost.EventType = ES_PairButtonPushed;
      PostTUGCommComm(toPost);
      break;
    }
